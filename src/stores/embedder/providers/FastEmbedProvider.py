...
class FastEmbedProvider(EmbedderInterface):
    ...
    def embed_text(
        ...
        """
        Memory-optimized embedding with streaming and float16 conversion.
        """
        ...
        if document_type == 'query':
            embed_type = 'query'
        elif document_type in ('passage', 'document'):
            embed_type = 'passage'
        else:
            embed_type = 'passage'
        ...
        try:
            ...
            # MEMORY OPTIMIZATION: Convert to float16 and process in streaming fashion
            # This reduces memory by 50% (float32 -> float16)
            embeddings = []
            for emb in embeddings_generator:
                # Convert numpy array to float16, then to list
                # This halves the memory usage per embedding
                emb_float16 = emb.astype(np.float16).tolist()
                embeddings.append(emb_float16)
